...
TEST(test_pow, pow_q3)
{
	...
	v = pow(0.49276399445745422412, -5.8835245043058552739);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 64.322981851018752764, v);
	v = pow(-6.4752409516897069253, -6.8751864979277730328);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, -2.4444705813321956953e-06, v);
	v = pow(-5.8715105334709294382, -5.2844719141202443069);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, -5.4263706143834349635e-05, v);

	v = pow(-2.1887159738867450582e-20, -0.071450007903769652273);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 24.75480160483328973, v);
	v = pow(7.5288361004038776962e-21, -6.5131336485563835481);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 1.1629499005519533374e+131, v);
	v = pow(-1.410448516691137679e-20, -1.3642706758028513292);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, -4.9915099525866641171e+26, v);

	v = pow(-4.1855872517292573388e-250, 0.79817454805570964282);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, -7.2238761683175469583e-200, v);
	...
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 1, v);
	v = pow(5.4758412426314965446e-252, -6.9073434302798448824e-20);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 1, v);
	...
	v = pow(-1.8770793078566427632e-250, 54636248745367052288);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 0, v);
	...
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, INFINITY, v);
	v = pow(1.6790740176497198987e-251, -4.9643685896137380349e+250);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, INFINITY, v);
	...
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 0, v);
	v = pow(4.283774977036438758e+24, -9.3989247149375553557e+249);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 0, v);

	v = pow(8.93734396261979825e+249, -6.1775308077885758121e+250);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 0, v);
	v = pow(-5.7326553094105565209e+249, 2.5303586595925594717e+249);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, Inf, v);
	v = pow(-1.4589351967033603759e+250, -2.6484613347499315952e+250);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 0, v);

	v = pow(-1.6567426676199922999e+250, -1.3787745271033733825e-25);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 1, v);
	v = pow(5.0363822944293884449e+249, -1.3859517751021109387e-25);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 1, v);
	v = pow(6.2918988081455394637e+249, -3.04342513966162363e-25);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 1, v);

	v = pow(2.808667937912660912e+249, 3.5310061495749067598e-251);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 1, v);
	v = pow(-5.2179242923915455226e+250, -6.2410779537885673256e-250);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 1, v);
	v = pow(-3.2701756141360014448e+250, 1.2676630366033148722e-251);
	TEST_ASSERT_DOUBLE_WITHIN(getDoubleMaxAccuracy(v) * 2, 1, v);
}
